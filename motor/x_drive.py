...
#                   ||  DISPLAY=:0.0
#   2021/04/11      ||  基于新加速算法，调整代码调用，整理了代码
#########################################################
"""
    调用该模块中的 move 实现x轴的运动控制，
    目前规定x轴中线向开关方向为负方向
...
def move(distance, rpm_max_specify=None):
    """
    x轴依据距离及最高转速的设定，计算出行对应的步数，调用脉冲控制函数实现运动控制
    ...
    if rpm_max_specify:
        if 1 <= rpm_max_specify <= RPM_MAX:
            ...
            print("############################################################################################")
            print("x轴最高转速设定值不在范围[%s, %s]rpm内，不符合要求，强制设定为 %s rpm" % (RPM_MIN, RPM_MAX, RPM_MAX))
            print("############################################################################################")
            rpm_max = RPM_MAX
    else:
        rpm_max = RPM_MAX
    ...
    one_step(direction, step_num_value, rpm_max)                            # X轴运动
...
def position_fine_tuning(distance):
    """
    位置微调时调用该函数，微调距离在0-2mm之内,步进单位约是50um，5丝
    ...
    """
    if 0 < abs(distance) <= 2:
        step_num_value, unit_step = move(distance, 100)    # 微小的移动使用较大的脉冲宽度是的机械振动减小
