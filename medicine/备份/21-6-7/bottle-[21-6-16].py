...
TAKE_MEDICINE_ANGLE_PUSH_OUT = -42      # 设定降低出药口将药物顶出的角度

# take_medicine()
COUNT_CHECK_MAX = 50                     # 设定检查出药口准备药物情况的最大次数
BACK_MEDICINE_ANGLE = 70                # 设定回药的角度，使得药物回到瓶口附近但未回到瓶内
# ########################################################################
...
def turn_y_z(angle):
    ...
    #   2.3 执行z轴控制
    go.only_z(the_top_body_push_rod, target_z, 30)
    time.sleep(0.1)
...
def turn_z(angle):
    ...
    the_top_body_push_rod = coordinate_converter.body_push_rod("the_top")
    # 3、执行z轴控制
    go.only_z(the_top_body_push_rod, target_z, 15)
    time.sleep(0.1)
    # 4、更新目前角度
    global angle_current
    angle_current = angle
